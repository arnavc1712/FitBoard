...
const get_waypoints_info = async (source, end_point, distance) => {
    ...
    console.log("INSIDE WAYPOINTS INFO ", GET_WAYPOINTS_URL);
...
export const GetPaths = (source, distance) => {
    ...
    console.log("In getPaths", GET_END_POINTS_URL);
    let suggested_end_point = [];
    try{
        fetch(GET_END_POINTS_URL)
        .then((response) => {
            return response.json();
        })
        .then(async (data) => {
            let end_points = data.results;
            await Promise.all(end_points.map(async (end_point) => {
